...
function processwebpackJs() {
  ...
  return gulp.src(['node_modules/babel-polyfill/dist/polyfill.js', config.roots.webpackJs]).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(webpack(require(config.webpackConfig))).pipe(gulp.dest(config.dist.assets));
}
