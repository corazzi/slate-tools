...
function processEs6Js() {
  ...
  return gulp.src(['node_modules/babel-polyfill/dist/polyfill.js', config.roots.es6Js]).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(babel({ presets: ['es2015'] })).pipe(uglify({
    mangle: true,
    compress: true,
    preserveComments: 'license'
