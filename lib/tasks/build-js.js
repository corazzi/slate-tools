...
function processEs6Js() {
  ...
  return gulp.src(['node_modules/babel-polyfill/dist/polyfill.js', config.roots.es6Js]).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(webpack(require(config.webpackConfig))).pipe(gulp.dest(config.dist.assets));
}
...
gulp.task('watch:es6-js', function () {
  chokidar.watch([config.roots.es6Js, config.src.es6Js], { ignoreInitial: true }).on('all', function (event, path) {
    messages.logFileEvent(event, path);
    processEs6Js();
  });
});