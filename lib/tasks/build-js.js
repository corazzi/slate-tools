...
function processEs6Js() {
  ...
  return gulp.src(['node_modules/babel-polyfill/dist/polyfill.js', config.roots.es6Js]).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(webpack(require('../../webpack.config'))).pipe(gulp.dest(config.dist.assets));
}
